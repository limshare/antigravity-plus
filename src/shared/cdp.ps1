function Get-AntigravityDevToolsTargets {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        try {
            ...
            $targets = if ($raw -is [System.Array]) { $raw } elseif ($raw.value) { $raw.value } else { @($raw) }
...
function Get-AntigravityActivePageTarget {
    ...
    $targets = Get-AntigravityDevToolsTargets -Port $Port -TimeoutSeconds $TimeoutSeconds
    $page = $targets | Where-Object {
        $_.type -eq 'page' -and (
            $_.url -like '*127.0.0.1*' -or
            $_.url -like 'app://*' -or
            $_.url -like 'plugin://*'
        )
    ...
    if (-not $page) {
        # Fallback to any page target if specific URL match not met
...
function Invoke-CdpWebSocket {
    ...
    try {
        $client.ConnectAsync([Uri]$WebSocketDebuggerUrl, $cts.Token).GetAwaiter().GetResult() | Out-Null
